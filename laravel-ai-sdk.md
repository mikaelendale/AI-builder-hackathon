...
If your application uses multiple participant model types, you should consider defining an [Eloquent morph map](/framework/docs/eloquent-relationships#custom-polymorphic-types) so that stored participant types are not coupled to your model class names.
...
The `FileStorage` tool factory allows you to give agents access to a Laravel [filesystem disk](/framework/docs/filesystem). The `all` method returns tools that allow the agent to list, read, inspect, generate URLs for, write, delete, and copy files on the given disk:
...
If your application uses [Laravel MCP](/framework/docs/mcp), you may give your agents tools exposed by [Model Context Protocol](https://modelcontextprotocol.io) servers. Using the [Laravel MCP client](/framework/docs/mcp#client), you may connect to a remote or local MCP server and pass its tools directly to your agent.

> [!NOTE]
> MCP tools require the [Laravel MCP](/framework/docs/mcp) package to be installed in your application.
...
The AI SDK automatically wraps each MCP tool so the agent can call it like any other tool. You may also use a [named MCP client](/framework/docs/mcp#named-clients):
...
Or connect to a [local MCP server](/framework/docs/mcp#client-connecting):
...
For more information on creating and authenticating MCP clients, including bearer tokens and OAuth, consult the [MCP client documentation](/framework/docs/mcp#client).
...
The Laravel AI SDK dispatches a variety of [events](/framework/docs/events), including:
